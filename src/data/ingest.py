...
def download_and_merge_safe():
    ...
    for i, dataset_id in enumerate(DATASETS, start=1):
        ...
        except HTTPError as e: # checks if there are permissions needed to access dataset on Kaggle
            if e.response.status_code == 403:
                print(f"⚠ Skipping {dataset_id}: Forbidden (may require manual acceptance)")
            else:
